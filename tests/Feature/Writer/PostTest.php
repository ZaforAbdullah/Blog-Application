...
class PostTest extends TestCase
{
    ...
    public function test_unique_slug_and_required_validation_for_updating_a_post_by_owner()
    {
        ...
        $res->assertSessionHasErrors('title'); // It validates the rest of the fields and gives an error, but I only put the title
        $res->assertSessionHasErrors('slug');  // Slug, because it uses slug post_2, it gives an error of non-uniqueness
    ...
    public function test_updae_a_post_by_owner()
    {
        ...
        $old_cat_id = $post->categories[0]->id; // Since the relationship is many to many, we wanted to have a category in advance so that we can fully check the sync command
